  ...
  config = lib.mkIf cfg.enable {
    programs.zsh = {
      ...
      shellAliases = {
        ...
        nrs = "nix run nix-darwin -- switch --flake /Users/adrianooliviero/.config/dotfiles#mac";
    ...
    xdg.configFile."zsh/utils.zsh".text = ''
      ...
      clnipc() {
        for i in $(ipcs -s | awk '{print $2}' | grep -vE "[^0-9]"); do ipcrm -s $i; done
        for i in $(ipcs -q | awk '{print $2}' | grep -vE "[^0-9]"); do ipcrm -q $i; done
        for i in $(ipcs -m | awk '{print $2}' | grep -vE "[^0-9]"); do ipcrm -m $i; done
      }
